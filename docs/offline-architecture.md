...
| Online tafsirs (10 sources)                                                                                | OPTIONAL ONLINE | everything else works                                                     | that tafsir text                   | calm error + retry (`qr.tafsirFail`), no crash, 10s bound (`net.ts`) |
...
| Full 12-book hadith browser (+ lazy EN; Ahmed partial, Darimi AR-only)                                     | OPTIONAL ONLINE | search/favs of loaded books work                                          | uncached book editions             | calm failed card + retry (`hf.failT/S`), 10s bound                   |
