...
chromium`), not on every start. Verified 2026-09-14: `docker build`
succeeds and the full e2e suite passes inside the container as-is
(Playwright launches its own Chromium flags; no `--no-sandbox`
workaround needed). Never run `npm ci` inside a bind-mounted Linux
container against the host tree: it replaces Windows `node_modules`
shims with Linux ones and breaks host tooling — reinstall with host
`npm ci` if that happens. `docker build` requires LF line endings in the
Dockerfile (enforced by `.gitattributes` — drag-copied trees may carry
CRLF, re-checkout instead).
