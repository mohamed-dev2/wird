...
Priority: P0 (blocker), P1 (high), P2 (medium), P3 (low). Owners are the person
...
| TD-4 | e2e suite flaky under parallel workers (serial pinned in `npm run test:e2e`) | CI slower; parallelism unsafe                           | Unknown shared-state in specs                       | Release friction                          | Root-cause parallel flakiness, then unpin serial                                                                  | P1       |
...
| #    | Problem                                  | Resolution                                                                                                    |
| ---- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| TD-6 | `WIRD_APP_VERSION` drift vs package.json | `docs:check` check 7 asserts equality in CI (`docs:check — version stamp matches`); drift now fails the build |
